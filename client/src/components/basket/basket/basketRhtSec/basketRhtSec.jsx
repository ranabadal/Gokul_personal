...
//       } else {
//         setToast("Failed to place order.", "error");
...
export default function OrderSummary({ cartItems = [], addresses, customTitle, customTotalHeading, updateCartItems, showQuantityInput = false }) {
  ...
  // New function added to call the deletion API for a given product
      ...
      if (response.status === 201) {
        ...
        await Promise.all(
          cartItems.filter(item => item.checked)
            .map(item => handleRemoveFromCart(item.productId._id))
        );

        // Remove only ordered items from the cart state, keeping the user on the same page
        const updatedCart = cartItems.filter(item => !item.checked);
        updateCartItems(updatedCart); // Ensure `updateCartItems` updates the cart state
      } else {
        setToast("Failed to place order.", "error");
