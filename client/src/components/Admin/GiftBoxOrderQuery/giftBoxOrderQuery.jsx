        ...
        <table className={styles.orderTable}>
          <thead>
            <tr>
              ...
              <th>Items</th>
              <th>Matching Handbags</th>
              <th>Selected Sweets</th> {/* ✅ New column */}
              <th>Address</th>
              <th>Total Price</th>
              <th>Status</th>
              <th>Actions</th>
            ...
            {orders.length > 0 ? (
              orders.map((order) => (
                <tr key={order._id}>
                  ...
                  <td>
                    {order.cartItems.map((item) =>
                      item.selectedSweets && item.selectedSweets.length > 0 ? (
                        item.selectedSweets.map((sweet, sIdx) => (
                          <div key={sIdx}>
                            {sweet.name} - ₹{sweet.price}
                          </div>
                        ))
                      ) : (
                        <div>No sweets selected</div>
                      )
                    )}
                  </td>
                  <td>
                  ...
                  <td>{order.status}</td>
                  <td>
                    <button className={styles.approveBtn} onClick={() => handleStatusUpdate(order._id, order.status, "Approved")}>Approve</button>
                    <button className={styles.rejectBtn} onClick={() => handleStatusUpdate(order._id, order.status, "Rejected")}>Reject</button>
                    <button className={styles.completeBtn} onClick={() => handleStatusUpdate(order._id, order.status, "Completed")}>Complete</button>
                    <button className={styles.editBtn} onClick={() => handleEditOrder(order)}>Edit</button>
                  </td>
                </tr>
              ))
