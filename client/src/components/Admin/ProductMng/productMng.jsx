...
const ProductManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    rating: '',
    reviewCount: '',
    image: null, // Store the File object instead of a base64 string
    discountPrice: '',
    oldPrice: '',
    discountPercent: '',
    isTodaysDeal: false,
  ...
  // Input handler for text, checkboxes, and other form inputs
  const handleInputChange = (e) => {
    const { name, value, type, checked } = e.target;
    setFormData(prevState => ({
      ...prevState,
  ...
  // File change handler: store the selected File object in state
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
  ...
  // Add product handler using FormData to include file upload
  const handleAddProduct = async (e) => {
    e.preventDefault();
    try {
      ...
      data.append('isTodaysDeal', formData.isTodaysDeal);
      data.append('bulkOrderAvailable', formData.bulkOrderAvailable);
  ...
  // Update product handler using FormData (only send new image if selected)
  const handleEditProduct = async (e) => {
    e.preventDefault();
    try {
      const data = new FormData();
      ...
      data.append('rating', formData.rating);
      data.append('reviewCount', formData.reviewCount);
      // Append image only if a new File is selected (instanceof File)
      ...
      if (result.success) {
        setProducts(products.map(product => (
          product._id === editProductId ? result.product : product
        )));
        setEditProductId(null);
        setFormData({
          category: '',
  ...
  // Populate the form with the product details when editing
  const handleEditClick = (product) => {
    setEditProductId(product._id);
    setFormData({
      category: product.category,
      ...
      rating: product.rating,
      reviewCount: product.reviewCount,
      image: product.image ? product.image.url : null, // Use the stored Cloudinary URL for preview
      discountPrice: product.discountPrice || '',
      oldPrice: product.oldPrice || '',
      discountPercent: product.discountPercent || '',
      isTodaysDeal: product.isTodaysDeal || false,
        ...
        <tbody>
          {computedFilteredProducts.map(product => (
            <tr key={product._id}>
              ...
              <td>{product.bulkOrderAvailable ? 'Yes' : 'No'}</td>
              <td>
                <img className={styles.imgg} src={product.image?.url} alt={product.name} width="50" />
              </td>
              <td>
                <button className={styles.button} onClick={() => handleEditClick(product)}>
                  Edit
