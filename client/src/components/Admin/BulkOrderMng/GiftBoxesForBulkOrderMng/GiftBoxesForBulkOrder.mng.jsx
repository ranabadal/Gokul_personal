...
const handleAddCategory = async () => {
  try {
    ...
    // Expect newCategory.image to be a File object.
    if (newCategory.image instanceof File) {
      formData.append("image", newCategory.image);
    } else {
      return console.error("Image file is required for category.");
    }
    await axios.post(`${BASE_URL}/api/bulkorders/categories`, formData, {
...
const handleAddGiftBox = async () => {
  try {
    ...
    // Append the main gift box image if it exists (should be a File object)
    if (newGiftBox.image instanceof File) {
      formData.append("image", newGiftBox.image);
    } else {
      return console.error("Gift box image file is required.");
    }
    // For matching handbag, if applicable
    if (newGiftBox.hasMatchingHandbag) {
      // Copy matchingHandbag data except the image file (if any)
      const matchingData = {
      ...
      // Append matching handbag image if provided (should be a File)
      if (newGiftBox.matchingHandbag.image instanceof File) {
        formData.append("matchingHandbagImage", newGiftBox.matchingHandbag.image);
      }
      // Append the matchingHandbags array as a JSON string
      formData.append("matchingHandbags", JSON.stringify([matchingData]));
    }
    await axios.post(`${BASE_URL}/api/bulkorders/giftBoxes`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    setNewGiftBox({
      name: "",
...
const handleAddHandbag = async () => {
  try {
    ...
    if (newHandbag.image instanceof File) {
      ...
      return console.error("Handbag image file is required.");
    }
    await axios.post(`${BASE_URL}/api/bulkorders/generalHandbags`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    setNewHandbag({ name: "", category: "", image: "", price: "", minOrderQuantity: "" });
    fetchData();
...
const handleUpdateCategory = async () => {
  try {
    ...
    await axios.put(`${BASE_URL}/api/bulkorders/categories/${editId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    setNewCategory({ name: "", image: "" });
    setEditMode(false);
...
const handleUpdateGiftBox = async () => {
  try {
    ...
    await axios.put(`${BASE_URL}/api/bulkorders/giftBoxes/${editId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    setNewGiftBox({
      name: "",
...
const handleUpdateHandbag = async () => {
  try {
    ...
    await axios.put(`${BASE_URL}/api/bulkorders/generalHandbags/${editId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    setNewHandbag({ name: "", category: "", image: "", price: "", minOrderQuantity: "" });
    setEditMode(false);
...
const handleEdit = (item, type) => {
  ...
  setEditId(item._id);
  if (type === "category") {
    // Here we assume item.image is already an object from the backend
    setNewCategory({ name: item.name, image: item.image });
  } else if (type === "giftBox") {
    setNewGiftBox({
      name: item.name,
    ...
    <div className={styles.adminContainer}>
      ...
      <div className={styles.formContainer}>
        ...
        {selectedOption === "category" && (
          <div className={styles.formSection}>
            <h3>
              {editMode && editType === "category" ? "Edit Category" : "Add New Category"}
            </h3>
            <input
              type="text"
              placeholder="Category Name"
            ...
            {editMode && editType === "category" && newCategory.image && (
              <div className={styles.imagePreviewContainer}>
                {typeof newCategory.image === "object" && newCategory.image.url ? (
                  <img
        ...
        {selectedOption === "giftBox" && (
          <div className={styles.formSection}>
            <h3>
              {editMode && editType === "giftBox" ? "Edit Gift Box" : "Add New Gift Box"}
            </h3>
            <input
              type="text"
              placeholder="Gift Box Name"
            ...
            <input
              ...
              value={newGiftBox.minOrderQuantity}
              onChange={(e) =>
                setNewGiftBox({ ...newGiftBox, minOrderQuantity: e.target.value })
              }
            />
            ...
            {newGiftBox.hasMatchingHandbag && (
              <div className={styles.matchingHandbagDetails}>
                ...
                <input
                  ...
                  placeholder="Matching Handbag Min Order Quantity"
                  value={
                    (newGiftBox.matchingHandbag && newGiftBox.matchingHandbag.minOrderQuantity) || ""
                  }
                  onChange={(e) =>
                    setNewGiftBox({
                      ...newGiftBox,
            ...
            <tr key={cat._id}>
              ...
              <td>
                {cat.image && typeof cat.image === "object" && cat.image.url ? (
                  <img src={cat.image.url} alt={cat.name} className={styles.previewImage} />
                ) : (
                  "NA"
                )}
              </td>
            ...
            <tr key={box._id}>
              ...
              <td>
                {box.image && typeof box.image === "object" && box.image.url ? (
                  <img src={box.image.url} alt={box.name} className={styles.previewImage} />
                ) : (
                  "NA"
                )}
              </td>
            ...
            <tr key={hb._id}>
              ...
              <td>
                {hb.image && typeof hb.image === "object" && hb.image.url ? (
                  <img src={hb.image.url} alt={hb.name} className={styles.previewImage} />
                ) : (
                  "NA"
                )}
              </td>
